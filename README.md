...
- We highly recommend that production deployments include HTTPS Listeners, AWS Backup, AWS GuardDuty, AWS Web Application Firewall, and AWS Systems Manager Patch Manager, and [User Authentication](https://www.archivematica.org/en/docs/archivematica-1.13/admin-manual/security/security/#authentication-security). You must import into or generate a certificate with Amazon Certificate Manager to install a HTTPS Listener on an Application Load Balancer.
...
- `1` Application Load Balancer + `2` Target Groups + `2` Listeners
- `1` IGW + `2` NAT GW + `2` Public Subnets + `2` Private Subnets + `1` Private S3 VPC Endpoint
...
- `4` EC2 EBS Volumes 
    ...
    - Archivematica Data Volume (500 GB GP3 SSD/16000 IOPS/125 Mbps Throughput)  - /var/archivematica
        - NOTE: The Archivematica Data Volume must be 4x larger than the largest AIP that will be created. A 500 GB Data Volume will support a 120 GB AIP.
- `1` IAM Role (Archivematica Server's access to S3 Bucket and Systems Manager)
- `2` Network Security Groups (Application Load Balancer + EC2 Instance)
- `1` S3 Bucket (S3 Standard + Bucket Policy)
- `1` KMS Key used for S3 Bucket encryption
...
4. Periodically check to see if the Archivematica Services are up by navigating to ***http://ALB-URL:81***. You can also check on the status of the bootstrap by monitoring the /tmp directory for files beginning with bootstrap*. If this URL presents a configuration wizard, proceed to the next step.
